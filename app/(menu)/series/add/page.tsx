...
async function getTopic(key?:string) {
  if(!key) return;
  const res = await cacheRequest(API_URLS.priTopic + "/list/" + key, METHODS.GET);
  return response(res);
}
    ...
    <div className="flex items-center justify-center flex-col">
      ...
      <div className="w-full grid grid-cols-3 p-3 text-black sm:w-4/5 sm:px-1">
        <div className="flex justify-start items-end col-span-1">
          <Button onClick={() => router.push('/series')} text="<" type="ROUNDED" />
        </div>
      ...
      >
        ...
        <div key={`detail-form-input-site`} className={`col-span-12 sm:col-span-12`}>
          <label htmlFor="site" className="block mb-2 text-sm font-medium text-gray-900">
            사이트
          </label>
          <SelectInput input={siteSelect} setFunction={setSite} dataFunction={getSite} />
        </div>
        <div key={`detail-form-input-topic`} className={`col-span-12 sm:col-span-12`}>
          <label htmlFor="topic" className="block mb-2 text-sm font-medium text-gray-900">
            주제
          </label>
          <SelectInput input={topicSelect} dataFunction={getTopic} parentValue={site}/>
        </div>
        <div key={`detail-form-input-name`} className={`col-span-12 sm:col-span-12`}>
          <label htmlFor="topic" className="block mb-2 text-sm font-medium text-gray-900">
            제목
          </label>
