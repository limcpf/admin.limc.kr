...
export default function TextForm({ form }:Props) {
    ...
    const inputsClass = "w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:bg-white focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 disabled:bg-gray-200 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none"; 
     ...
     */
    const refresh = () => {
        if(!confirm("새로고침시 내용이 초기화 됩니다. 정말 새로고침 할까요?")) return;
        put();
    }
            ...
            {title 
                ? <div>
                    <div className="text-lg flex justify-between">  
                        ...
                        { setRefresh
                            ? <div onClick={() => {refresh()}}>
                                <IconButton><span className="text-blue-400">↺</span></IconButton>
                            </div>
                            : <div></div>
                        }
                    </div>
            ...
            <div className={className}>
                { 
                    inputs.map(({id, type, option = {}}, idx) => {
                        ...
                        return (<div
                                    key={`input-${idx}`}
                                    className={getClassInput(option)} 
                                > 
                                {(option.title) &&
                                    <label
                                        htmlFor={id} 
