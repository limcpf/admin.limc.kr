...
export default function ListFooter({list}: {
  ...
  return (<>
          <div 
            className={`${list.first && "hidden"} col-start-1 col-end-2 sm:col-start-4 sm:col-end-5 ${pageBtnClass} justify-end`}
          ...
          <div className="col-start-2 col-end-12  sm:col-start-5 sm:col-end-8 flex justify-center items-center my-3">
            <form onSubmit={su} className={"w-5/6 grid grid-cols-12"}>
              <div className="relative col-span-8 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-s-lg">
                ...
                <button type="submit" className="col-span-4 border border-gray-700 p-2.5 text-sm bg-gray-700 text-white rounded-e-lg focus:bg-gray-800">
                  이동
                </button>
            </form>
          </div>
          <div 
            className={`${list.last && "hidden"} col-start-12 col-end-13 sm:col-start-8 sm:col-end-9 ${pageBtnClass} justify-start`}
            onClick={() => router.push(`?page=${list.curPage+1}`)}
          >
            {`➡`}
          </div>
