...
export default function ListBody<T>({headers, data, index}:{
  ...
  const getHrefComponent = (href: string, id: keyof T, t: T, text: string,) => {
    ...
    return <Link className="font-medium text-blue-600 hover:underline" href={link}>
      {text}
    </Link>;
  };
  ...
  const getListFormBody = (h: ListFormHeader<T>, t: T) => {
    ...
    if (h.option && href) {
      ...
      if(!disabled && href) return getHrefComponent(href, hrefId, t, `${content}`);
    }
  ...
  return (
      <div className={`col-span-full grid grid-cols-12 py-2 ${divideRowBg(index)}`}>
